...
function create() {
  ...
  const checkAllowed8FrameVersions = (version) =>
    new Promise((resolve, reject) =>
      allowed8FrameVersions.includes(version)
        ? resolve(version === 'latest' ? LATEST_8FRAME : version)
        : reject(
            `${version} is an unsupported AFrame version: (${JSON.stringify(
              allowedVersions
            )})`
          )
    );
  ...
  const loadAFrameForXr = (args) => {
    ...
    return checkAllowed8FrameVersions(version)
      .then((ver) =>
        loadJsPromise(`//cdn.8thwall.com/web/aframe/8frame-${ver}.min.js`)
      )
...
const eagerload = () => {
  ...
  Object.keys(attrs).forEach((a) => {
    ...
    if (attr == 'xrextras-almost-there') {
      ...
      redirectMatch &&
        window.XRExtras.AlmostThere.configure({ url: redirectMatch[1] });
      window.XR8
